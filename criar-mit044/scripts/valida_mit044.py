...
def main():
    ...
    # travessão e meia-risca: a redação da MIT usa pontuação simples (vírgula,
    # dois-pontos, parênteses, hífen) para não ter cara de texto gerado
    com_traco = [t.strip()[:60] for t in tudo if '—' in t or '–' in t]
    ok(not com_traco, 'sem travessão (—) nem meia-risca (–) no corpo',
       '%d ocorrência(s), ex.: %r' % (len(com_traco), com_traco[:2]))
