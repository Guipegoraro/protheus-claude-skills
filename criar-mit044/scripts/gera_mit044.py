...
# documento e pasta-base do JSON: figuras e tabelas livres precisam do documento (para
# registrar a imagem como parte) e resolvem caminhos relativos a partir do JSON
_DOC = None
_BASE = '.'
LARGURA_MAX_CM = 17.5   # area util A4 do template (~505pt)
...
# ------------------------------------------------------------------ conteudo
def como_docx(el):
    """Reparseia um elemento clonado do prototipos.xml com o parser do python-docx,
    para poder usar Paragraph/Run (add_picture, alinhamento, fonte)."""
    return parse_xml(etree.tostring(el))


def paragrafo_imagem(protos, item):
    """Figura centralizada (parágrafo com a imagem) + legenda em itálico 9pt cinza."""
    src = item.get('src', '')
    caminho = src if os.path.isabs(src) else os.path.join(_BASE, src)
    if not os.path.isfile(caminho):
        raise ErroConteudo('imagem não encontrada: ' + caminho)
    largura = float(item.get('largura_cm', 16))
    if largura > LARGURA_MAX_CM:
        raise ErroConteudo('largura_cm %.1f excede a área útil (%.1f cm): %s'
                           % (largura, LARGURA_MAX_CM, src))
    p_el = como_docx(clona_paragrafo(protos['body'], ''))
    for r in p_el.findall(W_R):
        p_el.remove(r)
    p = Paragraph(p_el, _DOC._body)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.add_run().add_picture(caminho, width=Cm(largura))
    out = [p_el]
    legenda = item.get('legenda', '')
    if legenda:
        l_el = como_docx(clona_paragrafo(protos['body'], legenda))
        l = Paragraph(l_el, _DOC._body)
...
def tabela_livre(protos, linhas, contexto):
    """Tabela de N colunas no estilo da tabela de Anexos (1a linha = cabeçalho).
    Colunas são criadas/removidas a partir da última e as larguras rateadas."""
    if not linhas or not isinstance(linhas[0], (list, tuple)):
        raise ErroConteudo('tabela sem linhas em ' + contexto)
    n = len(linhas[0])
    if n < 1 or any(len(l) != n for l in linhas):
        raise ErroConteudo('todas as linhas da tabela precisam ter %d colunas em %s'
                           % (n, contexto))
    tbl = copy.deepcopy(protos['anexos'])
    grid = tbl.find(qn('w:tblGrid'))
    cols = grid.findall(qn('w:gridCol'))
    largura = sum(int(c.get(qn('w:w'))) for c in cols) // n
    for c in cols[1:]:
        grid.remove(c)
    cols[0].set(qn('w:w'), str(largura))
    for _ in range(n - 1):
        grid.append(copy.deepcopy(cols[0]))
    for tr in tbl.findall(qn('w:tr')):
        tcs = tr.findall(qn('w:tc'))
        while len(tcs) < n:
            novo = copy.deepcopy(tcs[-1])
            tcs[-1].addnext(novo)
            tcs.append(novo)
        for tc in tcs[n:]:
            tr.remove(tc)
        for tc in tcs[:n]:
            ...
            if w is not None:
                w.set(qn('w:w'), str(largura))
                w.set(qn('w:type'), 'dxa')
    return clona_tabela(tbl, [[str(c) for c in l] for l in linhas], _DOC)


def itens_para_elementos(itens, protos, contexto):
    """Converte a lista do JSON em elementos <w:p>/<w:tbl>. Item pode ser string
    (parágrafo) ou objeto {"tipo": "p"|"bullet"|"num"|"label", "texto": "..."},
    {"tipo": "imagem", "src": "...", "legenda": "...", "largura_cm": 16} ou
    {"tipo": "tabela", "linhas": [[cabeçalho...], [linha...], ...]}."""
    out = []
    n_num = 0
    for item in itens:
        ...
        if tipo == 'imagem':
            out += paragrafo_imagem(protos, item)
            continue
        if tipo == 'tabela':
            out.append(tabela_livre(protos, item.get('linhas'), contexto))
            continue
        if not texto:
...
def main():
    ...
    global _DOC, _BASE
    _DOC = doc
    _BASE = os.path.dirname(os.path.abspath(args.json))
