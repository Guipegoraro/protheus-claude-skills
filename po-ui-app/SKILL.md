...
Hybrid (same app, both modes) is supported: branch on `ProAppConfigService.insideProtheus()`. When the user hasn't specified the mode, ask, because licensing and deploy differ materially. Details: [references/protheus-embedded.md](references/protheus-embedded.md) and [references/authentication.md](references/authentication.md).
