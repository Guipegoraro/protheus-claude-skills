...
When reviewing SQL in the TOTVS Protheus ecosystem — whether embedded SQL (`TCQuery`, `TCSqlExec`), Workarea-based access, or standalone queries — apply the following additional checks.
...
| Complex joins across multiple tables | Embedded SQL (`TCQuery`) |
| Aggregate queries (SUM, COUNT, AVG) | Embedded SQL (`TCQuery`) |
| Bulk INSERT/UPDATE/DELETE operations | `TCSqlExec` |
| Reports with heavy filtering | Embedded SQL or temporary tables via `TCSqlExec` |
...
```advpl
// GOOD: Parameterized embedded SQL via TCQuery
...
- [ ] String values are sanitized to prevent SQL injection (use `FWPreparedStatement` — no raw user input concatenated)
...
| Concatenating user input into SQL | SQL injection vulnerability | Use `FWPreparedStatement` or `TcGenQry2` to parameterize queries |
...
- **Never concatenate raw user input** into SQL strings — use `FWPreparedStatement` or `TcGenQry2` for parameterized queries
