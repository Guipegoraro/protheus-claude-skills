---
name: query-builder
description: "Build optimized and safe SQL queries for Protheus tables. Automatically includes mandatory filters (D_E_L_E_T_, branch), suggests appropriate indexes from SIX patterns, generates both Embedded SQL (TCQuery) and Workarea (DBSelectArea/DBSeek) versions, and warns about common Protheus SQL pitfalls. Use when user says 'build query', 'SQL for Protheus table', 'TCQuery', 'Workarea vs SQL'."
license: MIT
metadata:
  domain: Protheus
...
- Building TCQuery/TCSqlExec queries in AdvPL/TLPP
...
| 2 — Simple Select (Embedded SQL) | `FWPreparedStatement` + `TCGenQry` | Aggregations, complex filters, no suitable index for Workarea |
| 3 — Multi-Table Join | Embedded SQL with `INNER JOIN` | Cross-table reporting, invoice details with product names |
| 4 — INSERT/UPDATE (TCSqlExec) | `FWPreparedStatement` + `TCSqlExec` | Direct SQL writes (bypasses data dictionary triggers) |
| 5 — Counting Records | `SELECT COUNT(*)` via `TCGenQry` | Record counts with filters |
...
| Criterion                            | Workarea (DBSeek)            | Embedded SQL (TCQuery)            |
| ------------------------------------ | ---------------------------- | --------------------------------- |
...
| Record locking for update            | **Required**                 | Use TCSqlExec (bypasses triggers) |
...
The safe pattern:
1. Create `FWPreparedStatement():New()`
2. Use `?` placeholders in the query string for all dynamic values
3. Bind values with `SetString()`, `SetNumeric()`, `SetDate()`, etc.
4. Execute via `TCGenQry(,, oStatement:GetFixQuery())` or `TCSqlExec(oStatement:GetFixQuery())`
...
> Read [references/query-patterns-and-examples.md](references/query-patterns-and-examples.md) for complete safe/unsafe code examples including LIKE clauses.
...
| Not closing TCQuery aliases                   | Memory leak, alias exhaustion                        | Always call `(cAlias)->(DBCloseArea())`                                               |
| Macro execution in SQL                        | SQL injection risk                                   | Use `FWPreparedStatement` for parameterized values                                    |
...
- [ ] User-provided values parameterized with `FWPreparedStatement`
- [ ] TCQuery alias created with `GetNextAlias()`
- [ ] TCQuery alias closed with `DBCloseArea()` in all code paths (including errors)
...
- **SQL injection via macro-execution**: Never use `&(cExpr)` to build SQL strings. Use `FWPreparedStatement` to parameterize all dynamic values in queries.
